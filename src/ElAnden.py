...
ventanaPrincipal.attributes("-fullscreen", False)
...
Padel = {}
Tenis = {}
# FIX: compras inicializado junto a las demás variables globales (antes estaba al final del archivo)
compras = []
...
# FIX: Atras() comparaba con strings incorrectos ("src/Imagenes/Fondos/HojasMenu" etc.)
# en lugar de "HojasMenu" y "HojasPrecios" como usa Siguiente(). Corregido.
def Atras(TipoHoja):
    global iMenu , FondoMenu , iPrecios ,  FondoPrecios
    if TipoHoja == "HojasMenu":
        iMenu -= 1
...
def TablaReservas(NombreArchivoReservas):
    ...
    BotonBuscar = ctk.CTkButton(FrameBusqueda, text="Buscar", width=175,
                                height=30, command=lambda: BuscarReserva(NombreArchivoReservas),
...
# FIX: BuscarReserva ahora recibe NombreArchivoReservas como parámetro
# en lugar de depender de una variable global indefinida en su scope.
def BuscarReserva(NombreArchivoReservas):
    global MailUsuario
...
def CancelarReserva(TipoReservaSeleccionado):
    ...
    if TipoReservaSeleccionado is None:
        ...
        return
    ventanaSeleccion.destroy()
    # FIX: rutas corregidas con el prefijo "src/JSON/" (antes eran "Reservas.json" y "ReservasRestaurante.json")
    if TipoReservaSeleccionado == "Canchas":
        NombreArchivoReservas = "src/JSON/Reservas.json"
    elif TipoReservaSeleccionado == "Restaurante":
        NombreArchivoReservas = "src/JSON/ReservasRestaurante.json"
...
def PantallaReservaRestaurante():
    ...
    def reservaconexito():
        ...
        if NombrePersona=="" and TelefonoPersona==""  :
        ...
        elif DiasData[DiaSeleccionado]["Lugares Disponibles"] < CantidadComensales:
            ...
            # FIX: messagebox.showinfo() siempre devuelve None, nunca un bool.
            # El bloque de guardado y envio de mail estaba dentro de "if respuesta:"
            # lo que hacia que nunca se ejecutara. Ahora se ejecuta directamente.
...
# FIX: messagebox.showinfo() requiere dos argumentos (titulo, mensaje).
# Antes: messagebox.showinfo(f"Compra registrada: {nombre} - {precio}") -> TypeError
def VerificarSaldo(precio, nombre):
    compras.append((nombre, precio)) 
    messagebox.showinfo("Compra registrada", f"Has agregado {nombre} al carrito.")
...
# FIX: ruta corregida de DeporteElegido + ".json" a "src/JSON/" + DeporteElegido + ".json"
def ActualizarCanchas(DeporteElegido):
    ...
    if DiaElegido != "Seleccione un Dia" and HorarioElegido != "Seleccione un Horario":
        NombreArchivo = "src/JSON/" + DeporteElegido + ".json"
        ListaCanchas = CargarArchivo(NombreArchivo)
        if not ListaCanchas:
            return
        CanchasDisponibles = MostrarCanchasDisponibles(ListaCanchas, DiaElegido, HorarioElegido)
...
def DesignarPrecios():
    ...
    # FIX: ruta corregida de "Precios.json" a "src/JSON/Precios.json"
    Archivo = CargarArchivo("src/JSON/Precios.json")
    for Datos in Archivo:
...
def VentanaPago(Precio):
    ...
    # FIX: ruta corregida de "src/JSON/reservas.json" (minúscula) a "src/JSON/Reservas.json"
